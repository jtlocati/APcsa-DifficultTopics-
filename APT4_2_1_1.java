...
public class APT45 {
    public static void main(String[] args){
        // For this exsample word = sunflower. and sub = low
    ...
    for (int i = 0; i < word.length(); i++){
        String letter = word.substring(i, i + sub.length());            // i, says that we start at point i (0 origonaly). sub.length is saying that our substring is spanning sub.length (3 char's). while i makes sure that our sub.length kkeps moving down the substring
            if (letter.equals(sub)){            //checks that sub == letter
                IsFound = true;             //changes bool to true if found
                break;          //this makes the "<=" sighn on line 12 non-essencal
                } 
        }
